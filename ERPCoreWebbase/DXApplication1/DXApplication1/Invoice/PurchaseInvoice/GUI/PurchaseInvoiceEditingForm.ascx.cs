...
using WebModule.Invoice.PurchaseInvoice.State;
...
{
    public partial class PurchaseInvoiceEditingForm : System.Web.UI.UserControl
    {
        ...
        #region UpdateGUI
        public bool PurchaseInvoiceCanceling_UpdateGUI()
        {
            popupInvoiceEditingForm.ShowOnPageLoad = false;
            return true;
        }
        public bool PurchaseInvoiceCreating_UpdateGUI()
        {
            popupInvoiceEditingForm.ShowOnPageLoad = true;
            popupInvoiceEditingForm.HeaderText = "Thông tin phiếu mua hàng - Thêm mới";
        ...
        #region CRUD
        public bool PurchaseInvoiceCanceling_CRUD()
        {
            return true;
        }
        ...
        public bool PurchaseInvoiceEditing_CRUD()
        {
            //Read bill data
            formlayoutInvoiceEditingForm_LoadData();
        ...
        public bool PurchaseInvoiceLocked_CRUD()
        {
            //Read bill data
            formlayoutInvoiceEditingForm_LoadData();
        ...
        public bool PurchaseInvoiceCanceling_PreTransitionCRUD(string transition)
        {
            ...
            using (UnitOfWork uow = XpoHelper.GetNewUnitOfWork())
            {
                try
                {
                    purchaseInvoiceBO.Delete(session, BillId);
                    uow.CommitChanges();
                    return true;
                }
                catch (Exception)
        ...
        public bool PurchaseInvoiceCreating_PreTransitionCRUD(string transition)
        {
            ...
            if (transition.ToUpper().Equals("SAVE"))
            {
                using (UnitOfWork uow = XpoHelper.GetNewUnitOfWork())
                {
                    try
                    {
                        ...
                        NAS.BO.Invoice.PurchaseInvoiceBO purchaseInvoiceBO = new NAS.BO.Invoice.PurchaseInvoiceBO();
                        purchaseInvoiceBO.Save(uow, BillId, code, issuedDate, sourceOrganization, targetOrganization);
                        ...
                        if (comboBuyer.Value != null)
                        {
                            buyerId = (Guid)comboBuyer.Value;
                        }
                        ...
                        purchaseInvoiceBO.UpdateBillActor(uow, BillId, creatorId, buyerId, chiefAccountantId, directorId);
        ...
        public bool PurchaseInvoiceEditing_PreTransitionCRUD(string transition)
        {
            //Save data when transition is SAVE
            if (transition.ToUpper().Equals("SAVE"))
            {
        ...
        public bool PurchaseInvoiceLocked_PreTransitionCRUD(string transition)
        {
            return true;
        }
        #endregion
