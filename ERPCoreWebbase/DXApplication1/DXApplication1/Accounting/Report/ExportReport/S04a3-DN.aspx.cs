...
{
    public partial class S04a3_DN : System.Web.UI.Page
    {
        Session session;
        ...
        public void load_combobox_month()
        {
            ...
            xComboBox_month.SelectedIndex = 0;//hien thi tren giao dien
        ...
        public void load_combobox_owner()
        {
            ListEditItem defaultItem = new ListEditItem("Select One", -1);
            xComboBox_Owner.Items.Insert(0, defaultItem);
            xComboBox_Owner.SelectedIndex = 0;

            XPView view = new XPView(session, typeof(OwnerOrgDim));
            view.Properties.AddRange(new[] { new ViewProperty("name", DevExpress.Xpo.SortDirection.None, "[Name]", false, true) });
            foreach (ViewRecord mini_view in view)
            {
                xComboBox_Owner.Items.Add(mini_view[0].ToString(), mini_view[0].ToString());
            }
