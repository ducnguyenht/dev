...
{
    public class RPT_OutputInventoryCommand : RPT_InventoryCommand
    {
        public override void LoadInventoryCommandReport(Guid InventoryCommandId)
        {
            ...
            try
            {
                ...
                if (!command.CommandType.Equals('O'))
                ...
                Person pReceiver = ICBO.GetSelectedActorInventoryCommandCombobox(InventoryCommandId, DefaultInventoryCommandActorTypeEnum.RECEIVER);
                ...
                if (pReceiver != null)
                    pReceiver = uow.GetObjectByKey<Person>(pReceiver.PersonId);

                RPT_InventoryCommand_Rows =
                    retJournal2.Select(
                        i => new RPT_OutputInventoryCommand_Row()
                        {
                            ...
                            SalesInvoiceCode = bill == null ? string.Empty : bill.Code,
                            SalesInvoiceDate = command.IssueDate,
                            ReceiverName = pReceiver == null ? string.Empty : pReceiver.Name,
                            RelevantDepartment = pReceiver != null && pReceiver.DepartmentPersons.Count > 0 && pReceiver.DepartmentPersons[0]
                                != null && pReceiver.DepartmentPersons[0].DepartmentId != null ? pReceiver.DepartmentPersons[0].DepartmentId.Name : string.Empty,
                            CreatorName = pCreator == null ? string.Empty : pCreator.Name,
                            StoreKeeperName = pStoreKeeper == null ? string.Empty : pStoreKeeper.Name,
                            BussinessReason = command.Description,
